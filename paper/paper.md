...
![Map layers for terrain assessment. Example of multi-robot exploration: [Crex](https://robotik.dfki-bremen.de/de/forschung/robotersysteme/crex) explores the environment, collects soil samples, and generates both a multi-level surface (MLS) map and a shared soil map. Left: MLS converted into a Crex traversability map with smaller obstacle inflation due to the small robot size. Middle: Soil map with sand (yellow), gravel (green), and unknown (light blue), shared between Crex and Artemis. Right: MLS converted into an [Artemis](https://robotik.dfki-bremen.de/de/forschung/robotersysteme/artemis) traversability map fused with soil information, where sand is marked as obstacle (red) because Artemis, unlike the more mobile Crex, treats sand as non-traversable, with larger inflation adjusted for its bigger footprint.](figures/soilmap.png)
